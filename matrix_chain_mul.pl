...
my $subroutines = << 'END_SUB';
	...
	sub to_excel_pos{#expects a ref to array names and a string to excelify
		...
		while($string =~ m!\$(\w*)\[(\d+)\]\[(\d+)\]!g){
			if (defined get_index($input,$1)){
				my $index = get_index($input,$1);
				my $temp = convert($$input[$index][2]+$3).($$input[$index][1]+$2+1);
				$string =~ s!\$$1\[(\d+)\]\[(\d+)\]!$temp!;
			}
		}
		while($string =~ m!\$(\w*)\[(\d+)\]!g){
			if (defined get_index($input,$1)){
				my $index = get_index($input,$1);
				my $temp = convert($$input[$index][2]+$2).($$input[$index][1]+1);
				$string =~ s!\$$1\[(\d+)\]!$temp!;
			}
		}
		return $string;
